...
The system SHALL provide an Orders (`Tilaukset`) tab in primary app navigation so users can access order browsing directly. In the redesigned tab layout, this Orders tab SHALL remain directly visible alongside `Varasto`, `Asiakkaat`, and `Raportti`.
...
The system SHALL allow users to open an order details/actions view from an order row tap, and order detail SHALL list ordered products with line `name` and `amount`, SHALL display customer information, and SHALL display delivery method and selected point information.
...
#### Scenario: Order detail shows delivery method
- **WHEN** user opens order detail page for an order that has `deliveryMethod`
- **THEN** system displays selected delivery method value

#### Scenario: Order detail fetches selected point information
- **WHEN** order has company and selected point identifier
- **THEN** system fetches point information from `${EXPO_PUBLIC_FIREBASE_API}/orders/company/:company/point/:id`

#### Scenario: Order detail shows selected point information
- **WHEN** point fetch succeeds
- **THEN** system displays selected point information on order detail page

#### Scenario: Order detail handles point fetch failure
- **WHEN** point fetch fails or point identifier is missing
- **THEN** system shows non-blocking fallback message for point information section
