...
The system SHALL keep `Varasto`, `Tilaukset`, `Asiakkaat`, and `Raportti` as visible primary tabs in home navigation.

#### Scenario: Primary tabs visible after redesign
- **WHEN** user views the home tab bar
- **THEN** `Varasto`, `Tilaukset`, `Asiakkaat`, and `Raportti` are directly visible as tabs
...
- **THEN** system opens a popover showing menu options for `Kategoriat`, `Kampanjat`, and `Profiili`
